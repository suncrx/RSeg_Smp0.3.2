# -*- coding: utf-8 -*-
"""
Created on Thu May  2 17:38:28 2024
...
class SKLayer(nn.Module):
    def __init__(self, inplanes, planes, groups=8, ratio=16):
        super().__init__()
        ...
        self.split_5x5 = nn.Sequential(
            nn.Conv2d(inplanes, planes, kernel_size=3, padding=2, 
                      dilation=2, groups=groups),
            nn.BatchNorm2d(planes),
            nn.ReLU()
        )
...
if __name__ == "__main__":
    W, H = 128, 128
    chn = 32
    x = torch.randn(4, chn, H, W)
    attn = SKLayer(chn, chn)
    y = attn(x)
    print(x.shape)
    print(y.shape)